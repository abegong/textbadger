...
function loremIpsumSentence(numWords){
    var rval = "";
	rval += capitalize(getLoremWord()) + " ";
	rval += loremIpsum(numWords-1);
	rval += getLoremEnding();
	rval += " ";
    return rval;
}
...
function loremIpsumSentence2(numWords){
	...
	if(chance(50)){
		...
		rval += loremIpsum(part1);
		rval += ", ";
	} else {
		rval += " ";
	}
	rval += loremIpsum(numWords - part1 - 1);
	rval += getLoremEnding();
	rval += " ";
...
function loremIpsumParagraph(numWords){
	...
	while(words > 0){
		if(words > 10){
			...
			rval += loremIpsumSentence2(w);
			words = words - w;
		} else {
			rval += loremIpsumSentence2(words);
			words = 0;
		}
	}
    return rval;
